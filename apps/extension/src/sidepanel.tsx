...
function SidePanel() {
  ...
  return (
    <section className="extension-root"><VStack gap={5}>
      ...
      {captured?.extractedClaim || analysis?.extractedClaim ? <Card className="extension-card extension-generated" padding={4}><VStack gap={2}><Text type="supporting" color="secondary" className="extension-eyebrow">Key claim</Text><Text weight="semibold">{captured?.extractedClaim || analysis?.extractedClaim}</Text>{analysis?.summary ? <Text color="secondary">{analysis.summary}</Text> : null}</VStack></Card> : null}
      {counts ? (
        <section className="extension-metric-grid" aria-label="Evidence agreement">
          {([["supporting", "Supporting", counts.supporting], ["conflict", "Conflicting", counts.contradicting], ["inconclusive", "Inconclusive", counts.inconclusive]] as const).map(([tone, label, value]) => (
            <VStack gap={1} className="extension-metric" data-tone={tone} key={label}>
              <Text className="extension-metric-value">{value ?? 0}</Text>
              <Text type="supporting" color="secondary" className="extension-eyebrow">{label}</Text>
            </VStack>
          ))}
        </section>
      ) : null}
...
createRoot(document.getElementById("root")!).render(<React.StrictMode><Theme theme={threadStudioTheme} mode="light"><SidePanel /></Theme></React.StrictMode>);
